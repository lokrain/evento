...
export default async function HomePage({ params }: PageProps) {
  ...
  return (
    <div className="relative min-h-screen bg-background text-foreground">
              ...
              <Card key={card.title} className="hover-lift">
                <CardHeader>
                  <div className="flex items-center gap-2">
                    <Icon
                      className="h-4 w-4 text-primary icon-pop icon-pop-hover"
                      aria-hidden="true"
                    />
      ...
      <footer id="footer" className="mx-auto w-full max-w-6xl px-6 py-12">
        <div className="grid gap-6 border-t border-border pt-8 md:grid-cols-[1.5fr_1fr]">
          ...
          <nav className="grid gap-2 text-sm text-muted-foreground" aria-label="Footer links">
            {((t.raw("footer.links") as Array<{ label: string; href: string }>) ?? []).map(
              (link) => (
